...
DECLARE
    ...
    WHERE server_id = sserver_id AND prm.name='lt_stat_statements.max' AND stmt_cnt >= 0.9*cast(prm.setting AS integer)
...
RETURNS integer SET search_path=@extschema@ AS $$
    ...
    WHERE server_id = sserver_id AND name = 'lt_stat_statements.track'
