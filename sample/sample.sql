...
-- translate buffer size described in KB, MB and GB returned by current_setting() to number of bytes
-- setting for example: current_setting('shared_buffers'), current_setting('work_mem') and so on
CREATE FUNCTION format_buffer(IN setting text) RETURNS bigint SET search_path=@extschema@ AS $$
DECLARE
    substring_position integer;
    ret                bigint;
BEGIN
    SELECT upper(setting) INTO setting;

    IF (SELECT position('KB' IN setting) > 0) THEN
        SELECT position('KB' IN setting) INTO substring_position;
        SELECT substring(setting FROM 1 FOR substring_position - 1)::bigint * 1024 INTO ret;
    ELSIF (SELECT position('MB' IN setting)) > 0 THEN
        SELECT position('MB' IN setting) INTO substring_position;
        SELECT substring(setting FROM 1 FOR substring_position - 1)::bigint * 1024 * 1024 INTO ret;
    ELSIF (SELECT position('GB' IN setting)) > 0 THEN
        SELECT position('GB' IN setting) INTO substring_position;
        SELECT substring(setting FROM 1 FOR substring_position - 1)::bigint * 1024 * 1024 * 1024 INTO ret;
    ELSE
        ret := 0;
    END IF;
...
DECLARE
    ...
    shared_memory_kb     bigint;
    local_memory_kb      bigint;

    kernel_setting_array varchar[] := array[
        ...
        ['kernel.sched_migration_cost_ns',          '/proc/sys/kernel/sched_migration_cost_ns'],
        ...
        ['vm.nr_hugepages',                         '/proc/sys/vm/nr_hugepages'],
        ['fs.aio-max-nr',                           '/proc/sys/fs/aio-max-nr'],
        ...
        ['net.ipv4.tcp_tw_recycle',                 '/proc/sys/net/ipv4/tcp_tw_recycle'],
        ['net.ipv4.tcp_syn_retries',                '/proc/sys/net/ipv4/tcp_syn_retries'],
...
BEGIN
    ...
    */
    ...
    BEGIN
        collect_timings := current_setting('{lt_profile}.track_sample_timings')::boolean;
    EXCEPTION
    ...
    BEGIN
        ret := COALESCE(current_setting('{lt_profile}.max_sample_age')::integer);
    EXCEPTION
    ...
    FROM samples s LEFT OUTER JOIN  v_sample_settings prm ON
      (s.server_id = prm.server_id AND s.sample_id = prm.sample_id AND prm.name = '{lt_profile}.topn' AND prm.setting_scope = 1 AND NOT settings_refresh)
    ...
    CASE
      -- lt_stat_statements statistics collection
      WHEN (
        SELECT count(*) = 1
        FROM jsonb_to_recordset(server_properties #> '{extensions}') AS ext(extname text)
        WHERE extname = 'lt_stat_statements'
      ) THEN
    ...
    LOOP
      ...
      UPDATE tablespaces_list SET tablespacepath = current_setting('data_directory') || '/base' WHERE tablespacename = 'pg_default';
      UPDATE tablespaces_list SET tablespacepath = current_setting('data_directory') || '/global' WHERE tablespacename = 'pg_global';
    ...
    -- Sample new server_hardware stats
    IF (SELECT COUNT(*) FROM sample_server_hardware WHERE server_id = sserver_id) = 0 THEN
        ...
        SELECT
          ...
          mem.total_memory,
          mem.swap_total
        FROM pg_sys_cpu_info() cpu, pg_sys_memory_info() mem;
    ELSE
        UPDATE sample_server_hardware SET (
          sample_id,
          architecture,
          byte_order,
          cpus,
    ...
    -- Sample new cluster instance stats
    IF (SELECT COUNT(*) FROM sample_cluster_instance WHERE server_id = sserver_id) = 0 THEN
        ...
        SELECT
          sserver_id,
          s_id,
          (SELECT server_name FROM servers WHERE server_id = sserver_id) as hostname,
          current_setting('cluster_name') as clustername,
          CASE WHEN pg_is_in_recovery() = false THEN 'primary' ELSE 'standby' END as role,
          substring(version() from 1 for 17) as release,  -- for now, wait guc variable 'lightdb_server_version' added
          pg_postmaster_start_time()::timestamp(0) as startup_time;
    ELSE
        UPDATE sample_cluster_instance SET (
          sample_id,
          hostname,
          cluster_name,
          role,
    ...
    -- Delete obsolete wait event stats
    DELETE FROM sample_wait_event_total
    WHERE server_id = sserver_id
      AND (server_id, sample_id) NOT IN (
        SELECT server_id, sample_id FROM samples WHERE server_id = sserver_id
    );

    -- Sample new wait event stats
    SELECT sample_time INTO last_sample_time
    FROM samples
    WHERE server_id = sserver_id AND sample_id = s_id - 1;

    IF last_sample_time IS NULL THEN
        last_sample_time := now() + '-10 min';
    END IF;

    INSERT INTO sample_wait_event_total (
      server_id,
      sample_id,
      pid,
      queryid,
    ...
    SELECT
      ...
      count,
      state
    FROM sample_activity_profile t
    WHERE t.sample_time >= last_sample_time AND t.sample_time <= now()
    AND t.wait_event_type != 'Null'
    AND t.wait_event != 'Null';

    -- Sample new kernel settings stats
    IF (SELECT COUNT(*) FROM sample_kernel_settings WHERE server_id = sserver_id) = 0 THEN
        FOREACH r_result SLICE 1 IN ARRAY kernel_setting_array
        LOOP
            IF (SELECT pg_read_file(r_result[2], 0, 10240, true) IS NOT NULL) THEN
                ...
                (
                  ...
                  s_id,
                  r_result[1],
                  pg_read_file(r_result[2], 0, 10240, true)::text,
                  r_result[2]
                );
            END IF;
        END LOOP;
    ELSE
        ...
        LOOP
            IF (SELECT pg_read_file(r_result[2], 0, 10240, true) IS NOT NULL) THEN
                UPDATE sample_kernel_settings SET
                    sample_id = s_id,
                    name = r_result[1],
                    value = pg_read_file(r_result[2], 0, 10240, true)::text,
                    sourcefile = r_result[2]
                WHERE server_id = sserver_id
                AND   name = r_result[1];
            END IF;
        END LOOP;
    ...
    -- Delete obsolete cpu usage stats
    DELETE FROM sample_cpu_usage
    WHERE server_id = sserver_id
      AND (server_id, sample_id) NOT IN (
        SELECT server_id, sample_id FROM samples WHERE server_id = sserver_id
    );

    -- Sample new cpu usage stats
    ...
    SELECT
      sserver_id,
      s_id,
      cpu.usermode_normal_process_percent AS user_cpu,
      cpu.kernelmode_process_percent AS sys_cpu,
      cpu.idle_mode_percent AS idle_cpu,
      cpu.IO_completion_percent AS io_wait
    FROM pg_sys_cpu_usage_info() cpu;

    -- Delete obsolete database memory usage stats
    DELETE FROM sample_db_memory_usage
    WHERE server_id = sserver_id
      AND (server_id, sample_id) NOT IN (
        SELECT server_id, sample_id FROM samples WHERE server_id = sserver_id
    );

    -- Sample new database memory usage stats
    SELECT count(*) FROM pg_stat_activity WHERE backend_type = 'client backend'
    INTO client_connections;

    SELECT format_buffer(current_setting('shared_buffers')) +
           format_buffer(current_setting('wal_buffers'))
    INTO shared_memory_kb;

    SELECT (format_buffer(current_setting('work_mem')) +
            format_buffer(current_setting('temp_buffers')) +
            format_buffer(current_setting('maintenance_work_mem'))) * client_connections
    INTO local_memory_kb;
